...
def get_timestep_embedding(
    ...
    timesteps = timesteps.permute([0, 2, 3, 1])
    ...
    #     emb = torch.nn.functional.pad(emb, (0, 1, 0, 0), mode="constant")
    # emb = emb[0:100, :]
    emb = torch.nn.functional.pad(exponent, (0, 1), mode="constant")
    
    output = emb.reshape([1, -1])
    print("emb shape is: ", output.shape)
    return output



if __name__ == "__main__":
    timesteps_input = torch.randn(1, 3, 224, 224)
    net = Timesteps(3, False, 1)
    output = net(timesteps_input)
    
    # 导出模型到ONNX格式
    onnx_path = "/Users/bruce/PycharmProjects/Pytorch_learning/ONNX_Operator_Vis/ONNX_Operators/padding_tinymodel.onnx"
    torch.onnx.export(net, timesteps_input, onnx_path, verbose=True, opset_version=11)
